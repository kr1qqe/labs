...
private slots:
    ...
    void testAddCommand() {
        QString testName = "testAddCommand";
        testLogger->testStarted(testName);
        QElapsedTimer timer;
        timer.start();
        ...
        try {
            ...
            LessonModel::CommandResult result = model.ExecuteCommand("ADD 2024.12.25; 301; Иванов");

            QVERIFY(result.success);
            QCOMPARE(result.affectedRows, 1);
            QCOMPARE(model.Count(), 1);
            QCOMPARE(model.GetLesson(0).GetTeacher(), QString("Иванов"));
    ...
    void testRemCommand() {
        QString testName = "testRemCommand";
        testLogger->testStarted(testName);
        QElapsedTimer timer;
        timer.start();
        ...
        try {
            ...
            model.ExecuteCommand("ADD 2024.12.25; 301; Иванов");
            model.ExecuteCommand("ADD 2024.12.26; 302; Петров");
            QCOMPARE(model.Count(), 2);

            LessonModel::CommandResult result = model.ExecuteCommand("REM room == 301");

            QVERIFY(result.success);
            QCOMPARE(result.affectedRows, 1);
            QCOMPARE(model.Count(), 1);
            QCOMPARE(model.GetLesson(0).GetTeacher(), QString("Петров"));
    ...
    void testSaveCommand() {
        QString testName = "testSaveCommand";
        testLogger->testStarted(testName);
        QElapsedTimer timer;
        timer.start();

        try {
            QString filename = "temp_save_test.txt";

            LessonModel model;
            model.ExecuteCommand("ADD 2024.12.25; 301; Иванов");

            LessonModel::CommandResult result = model.ExecuteCommand(QString("SAVE %1").arg(filename));
