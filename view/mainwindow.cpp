...
void MainWindow::SetupUi() {
    ...
    input_text_edit_->setPlaceholderText(
        ...
        "ГГГГ.ММ.ДД Аудитория Преподаватель\n\n"
        "Примеры:\n"
...
void MainWindow::OnLoadFileClicked() {
    ...
    if (!path.isEmpty()) {
        ...
        QStringList lessons = model_->GetLessonsStringList();
        if (lessons.isEmpty()) {
            result_line_edit_->setPlainText("Файл загружен, но не содержит валидных занятий.\n\n"
                                            "Ошибки загрузки:\n" +
                                            model_->GetErrorLog().join("\n"));
        } else {
            result_line_edit_->setPlainText(lessons.join("\n\n"));

            if (!model_->GetErrorLog().isEmpty()) {
                QMessageBox::warning(this, "Предупреждение",
                                     QString("Загружено %1 занятий.\nБыло пропущено %2 строк с ошибками.\n\n"
    ...
    connect(exportButton, &QPushButton::clicked, [this, logDialog]() {
        ...
        if (!path.isEmpty()) {
            ...
            if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
                ...
                QStringList errors = model_->GetErrorLog();
                stream << "Лог ошибок парсера занятий\n";
...
void MainWindow::OnHelpButtonClicked() {
    ...
    browser->setHtml(
        "<h2>Справка</h2>"
        "<p><b>Формат ввода:</b> ГГГГ.ММ.ДД Аудитория Преподаватель</p>"
        "<p><b>Примеры:</b></p>"
        "<ul>"
        ...
        "<ul>"
        "<li>ADD date; room; teacher - добавить занятие</li>"
        "<li>REM условие - удалить занятия</li>"
        ...
        "<li>date > ГГГГ.ММ.ДД</li>"
        "<li>room == значение</li>"
        "<li>room contains значение</li>"
        "<li>teacher == значение</li>"
        "<li>teacher contains значение</li>"
        "</ul>"
...
void MainWindow::ProcessMultipleLines(const QString& data) {
    ...
    setWindowTitle(QString("Парсер - Успешно: %1, Ошибок: %2").arg(successCount).arg(errorCount));
}
...
void MainWindow::UpdateStatusBar() {
    if (status_label_) {
        status_label_->setText(QString("Всего занятий в модели: %1 | Ошибок в логе: %2")
                                   .arg(model_->Count())
                                   .arg(model_->GetErrorLog().size()));
    }
}
...
void MainWindow::ProcessCommandFile(const QString& filename) {
    ...
    QTextStream stream(&file);
    QList<LessonModel::CommandResult> results;
    int lineNum = 0;
    ...
    while (!stream.atEnd()) {
        ...
        LessonModel::CommandResult result = model_->ExecuteCommand(line);
    ...
    QString outputPath = "/Users/kr1qqe/Desktop/commands_result.txt";
    ...
    QMessageBox::information(this, "Готово",
                             QString("Команды выполнены.\n"
                                     "Результат сохранен на рабочий стол:\n%1\n\n"
                                     "Успешно: %2\nОшибок: %3")
                                 .arg(outputPath)
                                 .arg(std::count_if(results.begin(), results.end(),
                                                    [](const LessonModel::CommandResult& r) { return r.success; }))
                                 .arg(std::count_if(results.begin(), results.end(),
                                                    [](const LessonModel::CommandResult& r) { return !r.success; })));
}

void MainWindow::DisplayCommandResults(const QList<LessonModel::CommandResult>& results) {
    if (results.isEmpty()) {
        return;
    }
    ...
    content += QString("Всего занятий: %1\n").arg(model_->Count());

    QStringList lessons = model_->GetLessonsStringList();
    if (!lessons.isEmpty()) {
        content += QString("\nСписок занятий:\n");
        for (int i = 0; i < lessons.size(); ++i) {
            content += QString("%1. %2\n").arg(i + 1).arg(lessons[i]);
        }
    }
...
void MainWindow::SaveResultsToFile(const QList<LessonModel::CommandResult>& results, const QString& filepath) {
    QFile file(filepath);
    ...
    stream << "========================================\n";
    stream << QString("Всего занятий: %1\n\n").arg(model_->Count());

    QStringList lessons = model_->GetLessonsStringList();
    if (!lessons.isEmpty()) {
        stream << "Список занятий:\n";
        for (int i = 0; i < lessons.size(); ++i) {
            stream << QString("%1. %2\n").arg(i + 1).arg(lessons[i]);
        }
    } else {
        stream << "Список занятий пуст.\n";
    }
